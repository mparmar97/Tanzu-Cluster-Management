...
function routeListClusterGroups() {
  const router = express.Router();
  router.route('/')
    .get(async (req, res) => {
      const accessToken = await token().then((resp) => resp.accessToken);

      const listClusterGroups = await axios
        .get('https://capgeminietap.tmc.cloud.vmware.com/v1alpha1/clustergroups',
          {
            headers: {
      ...
      res.status(200).json({
        output: listClusterGroups,
      });
    });
  return router;
}

module.exports = routeListClusterGroups;
