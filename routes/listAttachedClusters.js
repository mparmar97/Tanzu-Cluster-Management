...
function routeListClusters() {
  const router = express.Router();
  router.route('/')
    .get(async (req, res) => {
      const accessToken = await token().then((resp) => resp.accessToken);

      const listClusters = await axios
        .get('https://capgeminietap.tmc.cloud.vmware.com/v1alpha1/clusters',
          {
            headers: {
...
module.exports = routeListClusters;
